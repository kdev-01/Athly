...
const FormEvent = ({ onEventCreated, eventToEdit }) => {
  ...
  const onSubmit = (data) => {
    if (eventToEdit) {
      axios
        .put(`http://localhost:8000/api/events/${eventToEdit.event_id}/`, data)
        .then(() => {
      ...
      <div className="border border-gray-300 rounded-lg overflow-hidden shadow-md w-full max-w-6xl mx-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-gray-50 p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                {...register("registration_start_date", {
                  required: "La fecha de inicio de inscripciones es obligatoria",
                })}
                className="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
              />
            ...
            <div>
              <label className="block text-gray-700 font-semibold">Fin de Inscripciones:</label>
              <input
                type="date"
                {...register("registration_end_date", {
                  required: "La fecha de fin de inscripciones es obligatoria",
                })}
                className="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
              />
